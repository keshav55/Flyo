...
- (void) doAction:(unsigned int)action
{
    ...
    ///NSLog(@"ANTES DE self.currentState ************");
   
    ///NSLog(@"CONTENIDO object: %@ ANTES DE LLAMAR A initValue ************", object);
    ////NSLog(@"CONTENIDO (NSString *)object: %@ ANTES DE LLAMAR A intValue ************", ((NSString *)object));
    ////NSLog(@"CONTENIDO ((unsigned int)(NSString *)object): %u ANTES DE LLAMAR A intValue ************", ((unsigned int)((NSString *)object)));
    
    
    /*****unsigned int miVar =  (unsigned int)[(NSString *)object intValue];
 NSLog(@"CONTENIDO miVar: %u ANTES DE LLAMAR A intValue ************", miVar);***/
    
    
    ///// NSLog(@"CONTENIDO DE currentState: %u ANTES DE LLAMAR A initValue ************", self.currentState);
   self.currentState = ((unsigned int)[(NSString *)object intValue]);
    /////self.currentState = miVar;  ////  = 7700832
    NSLog(@"CONTENIDO DE currentState: %u ************", self.currentState);
     NSLog(@"******LLEGA AL FIN DE doAction DE FiniteStateMachine ************");
}
